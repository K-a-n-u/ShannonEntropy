...
ShannonEntropy[base_?(Not[NumberQ[#] || # <= 0 || # == 1 &), distribution_] := Catch[Throw[Message[ShannonEntropy::errorBaseIsProblematic]; Return[$Failed]]];
ShannonEntropy[base_?(NumberQ[#] && 0 < # && # != 1 &), distribution_?DistributionParameterQ, OptionsPattern[]] := ShannonEntropy[distribution, optionsShannonEntropy, Method -> OptionValue[Method]]/Log[base];
ShannonEntropy[distribution_?DistributionParameterQ, OptionsPattern[]] := Function[Block[{#}, Return[OptionValue[Method][LogLikelihood[distribution, {#}], # \[Distributed] distribution]]]][Unique[]];
